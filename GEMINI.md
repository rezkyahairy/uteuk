...
tags: [agent, gemini-cli, ai, workflow]
---

# Gemini CLI Configuration for Obsidian Vault

> Context and rules for Gemini CLI when working with this Second Brain
...
- **Tag:** Add `gemini-processed` + `needs-review`
...
Gemini CLI loads the `/uteuk` command from `.gemini/commands/uteuk.toml`.
Reload with `/commands reload` after any changes.
...
**Agent Tags:**
- Processed tag: `gemini-processed`

## 🔄 Pipeline
All prompts live in `.uteuk/prompts/`. Pipeline commands are in `.uteuk/commands/uteuk.md`.
Agent-specific behavior is defined here in `GEMINI.md`.
...
| Tag | When to Apply |
|-----|---------------|
| `gemini-processed` | After you process a note |
...
---
*This configuration enforces the AI-Assisted Note-Taking Pipeline for Gemini CLI*
